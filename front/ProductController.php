...
class ProductControllerCore extends ProductPresentingFrontControllerCore
{
    ...
    public function initContent()
    {
        if (!$this->errors) {
			...
			{
				uasort($accessories, array($this, 'function_min_price'));
			...
			//{
			//	echo '<pre>'; 
				$coupon = Product::getCoupon($this->product->id, $this->context->language->id);
			...
			$this->assignCategoryData(20);
        }

        parent::initContent();
    }
	
	public function assignCategoryData($id_parent)
	{
		$sub_categories = Category::getSubCategoriesLite($id_parent);
		$product_categories = Product::getProductCategories($this->product->id);
		$collection_name = $collection_link = false;
		foreach($product_categories as $id_category)
		{
			if(in_array($id_category, $sub_categories) && empty($collection_name) && empty($collection_link))
			{
				$collection_name = Category::getNameById($id_category, $this->context->language->id);
				$collection_link = $this->context->link->getCategoryLink($id_category);
			}
		}
	...
	public function function_min_price($_a, $_b) 
	{

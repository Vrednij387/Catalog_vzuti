...
class AdminCartRulesControllerCore extends AdminController
{
    ...
    public function renderForm()
    {
        ...
        $this->context->smarty->assign(
            [
                ...
                'show_banner_value' => (int)$current_object->show_banner,
    ...
    public function displayDeleteLink(string $token, string $id, ?string $name = null): string
    {
        ...
        /**
     * Метод ініціалізації форми (якщо ваш PrestaShop ще обробляє $this->fields_form).
     * Якщо у вашій версії використовується renderForm() із Smarty — цей метод може не викликатися автоматично.
     */
    protected function initForm()
    {
        parent::initForm();

        // Додаємо поле "show_banner" у вкладку "information"
        // (якщо ваша тема/версія реально будує форму через $this->fields_form)
        $this->fields_form['information']['form']['input'][] = [
            'type'   => 'switch',
            'label'  => $this->l('Показати банер'),
            'name'   => 'show_banner',
            'is_bool' => true,
            'values' => [
                [
    ...
    /**
     * Метод, який зчитує POST-дані й зберігає значення show_banner у $object->show_banner
     */
    protected function copyFromPost(&$object, $table)
    {
        parent::copyFromPost($object, $table);
        if (Tools::getIsset('show_banner')) {
            $object->show_banner = (int)Tools::getValue('show_banner');
        }
    }
}

    ...
    {elseif $input.type == 'custom_button'}
        <div class="row">
            <button type="{if $input.submit}submit{else}button{/if}"
                    {if $input.class}class="{$input.class}"{/if} {*HTML*}
                    {if $input.name}id="{$input.name}"{/if} {*HTML*}
                    {if $input.name}name="{$input.name}"{/if} {*HTML*}
                    {if $input.onclick}onclick="{$input.onclick}"{/if}> {*HTML*}
                {$input.text|escape:'html':'UTF-8'}
            </button>
        </div>
    {else}

...
*}
<div class="tab_content tab_content_history">
    <div class="saved_settings_header">
        <div class="saved_settings_label">{l s='Last Executed Reports' mod='exportproducts'}</div>

        {if !empty($settings_list)}
            <div id="mpm_pe_clear_history">{l s='Clear History' mod='exportproducts'}</div>
        {/if}
    </div>
    <div class="saved_settings_content {if isset($settings_list) && $settings_list}active{/if}">
        {include file="{$path_tpl|escape:'htmlall':'UTF-8'}list-reports.tpl" product_reports=$settings_list setting_url=$setting_url}
    </div>

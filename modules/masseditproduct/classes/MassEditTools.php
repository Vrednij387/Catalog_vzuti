...
class MassEditTools
{
    ...
    {
        if (!Shop::isFeatureActive()) {
            Db::getInstance()->update('product', ['wholesale_price' => $price < 0 ? 0 : (float) $price], ' id_product = ' . (int) $id_product);
        }
        Db::getInstance()->execute('UPDATE `' . _DB_PREFIX_ . 'product_shop` ps
        LEFT JOIN `' . _DB_PREFIX_ . 'product` p ON ps.`id_product` = p.`id_product`
    ...
    {
        if (!Shop::isFeatureActive()) {
            Db::getInstance()->update('product_attribute', ['price' => (float) $price], ' id_product_attribute = ' . (int) $id_product_attribute);
        }
        ...
        SET pas.`price` = ' . (float) $price . ' WHERE pas.`id_product_attribute` = ' . (int) $id_product_attribute
    ...
    {
        if (!Shop::isFeatureActive()) {
            Db::getInstance()->update('product_attribute', ['wholesale_price' => $price < 0 ? 0 : (float) $price], ' id_product_attribute = ' . (int) $id_product_attribute);
        }
        Db::getInstance()->execute('UPDATE `' . _DB_PREFIX_ . 'product_attribute_shop` pas
        LEFT JOIN `' . _DB_PREFIX_ . 'product_attribute` pa ON pas.`id_product_attribute` = pa.`id_product_attribute`
        LEFT JOIN `' . _DB_PREFIX_ . 'product` p ON p.`id_product` = pa.`id_product`
    ...
    public static function updateObjectField($class_name, $field, $id, $value)
    {
        ...
        if (!$lang && $shop) {
            ...
            if ($field == 'weight' && $definition['table'] == 'product_attribute') {
    ...
    public static function renderMetaTag($meta, $id_product, $id_lang)
    {
        ...
        $feature_matches = (isset($matches[0]) && is_array($matches[0]) ? $matches[0] : []);
        foreach ($feature_matches as $fm) {
            $replace[$fm] = '';
        }

        preg_match_all('/\{feature_value\_[0-9]+\}/', $meta, $matches);
        $feature_value_matches = (isset($matches[0]) && is_array($matches[0]) ? $matches[0] : []);
    ...
    public static function deleteFeatures($id_product, $ids_feature = [])
    {
        ...
        SpecificPriceRule::applyAllRules([(int) $id_product]);
        return $result;
    }
    ...
    public static function checkImage($image, $key)
    {
        ...
        if (function_exists('exif_imagetype')) {
            $check_image = in_array(exif_imagetype($image_item), [IMAGETYPE_JPEG, IMAGETYPE_GIF, IMAGETYPE_PNG]);
        } elseif (function_exists('finfo_open')) {
            $check_image = false;
            $finfo = finfo_open(FILEINFO_MIME_TYPE);
            $file_info = finfo_file($finfo, $image_item);
            foreach (['image/jpeg', 'image/gif', 'image/png'] as $type) {
    ...
    private static $options_number_characters = [4 => '', 7 => '', 10 => '', 13 => '', 16 => '', 19 => ''];
    ...
    {
        return Shop::isFeatureActive() && Shop::getContext() == Shop::CONTEXT_SHOP ? (int) Context::getContext()->shop->id : 'p.id_shop_default';
    }
            ...
            foreach ($attributes as $attribute) {
                ...
                if (!array_key_exists(
                    ...
                    $combinations[$attribute['id_product_attribute']] = [
                        ...
                        'total_price_final' => ((int) Configuration::get('PS_TAX') ? $product_tax_calculator->addTaxes($attribute['price'] + $attribute['product_price']) : $attribute['price'] + $attribute['product_price']),

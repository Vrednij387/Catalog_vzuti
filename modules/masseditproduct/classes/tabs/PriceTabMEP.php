...
class PriceTabMEP extends BaseTabMEP
{
    ...
    public function __construct()
    {
        ...
        $this->price_whole = (int) Tools::getValue('price_whole');
    ...
    public function getQueryProducts($ids)
    {
        ...
        return is_array($query_products) ? $query_products : [];
    }
    ...
    public function applyChangeForProducts($products)
    {
        ...
        if ($this->checkAccessField('price')) {
                ...
                if ($this->type_price === self::TYPE_PRICE_WHOLESALE) {
                    if ($this->price_whole == 0) {
                        ...
                        if (version_compare(_PS_VERSION_, '1.7.6.9', '>=')) {
                            $return_products[$product['id_product']] = [
                    ...
                    if ($this->price_whole == 2) {
                        ...
                        $price = $product['wholesale_price'];
                        $price = MassEditTools::actionPrice($price, $this->action_price, $this->price_value);
                        $final_price = $price;
                        if ((int) Configuration::get('PS_TAX')) {
            ...
            foreach ($query_products as $query_product) {
                ...
                if (version_compare(_PS_VERSION_, '1.7.6.9', '>=')) {
                    $return_products[(int) $query_product['id_product']] = [
                        'price' => $this->context->currentLocale->formatPrice($price, $this->context->currency->iso_code),
                        'price_final' => $this->context->currentLocale->formatPrice($final_price, $this->context->currency->iso_code),
                    ];
                } else {
                    $return_products[(int) $query_product['id_product']] = [
                    'price' => Tools::displayPrice($price, $this->currency),
...
//                    MassEditTools::updateObjectField('Product', 'unit_price_ratio', (int) $query_product['id_product'], (float) $final_price);
            ...
            foreach ($query_products as $product_r) {
                ...
                $final_price = $product->getPrice(true, null, 6);
                if (version_compare(_PS_VERSION_, '1.7.6.9', '>=')) {
                    $return_products[$product_r['id_product']] = [
                        'price' => $this->context->currentLocale->formatPrice(
                            $price_round,
                            $this->context->currency->iso_code),
                        'price_final' => $this->context->currentLocale->formatPrice(
                ...
                if ($this->type_price === self::TYPE_PRICE_WHOLESALE) {
                    ...
                    if (array_key_exists(
                        ...
                        foreach ($combinations as $combination) {
                            ...
                            if (version_compare(_PS_VERSION_, '1.7.6.9', '>=')) {
                                $return_combinations[$combination['id_product_attribute']] = [
                                    'wholesale_price' => $this->context->currentLocale->formatPrice(
                    ...
                    if (array_key_exists(
                        ...
                        foreach ($combinations as $combination) {
                            ...
                            if (version_compare(_PS_VERSION_, '1.7.6.9', '>=')) {
                                $return_combinations[$combination['id_product_attribute']] = [
                                    'price' => $this->context->currentLocale->formatPrice($new_price_pa, $this->context->currency->iso_code),
                                    'total_price' => $this->context->currentLocale->formatPrice($product_price + $new_price_pa, $this->context->currency->iso_code),
                                    'price_final' => $this->context->currentLocale->formatPrice($price_pa_final, $this->context->currency->iso_code),
                                    'total_price_final' => $this->context->currentLocale->formatPrice($product_price_final + $price_pa_final, $this->context->currency->iso_code),
                                ];
                            } else {
                                $return_combinations[$combination['id_product_attribute']] = [
                    ...
                    if (count($combinations)) {
                        foreach ($combinations as $id) {
                            ...
                            if ($this->action_price == 1) {
                                if ($this->type_price == 1) {
                                    $rest = ($id['total_price_final'] + ($id['total_price_final'] / 100 * $this->price_value) - $id['product_price_final']) / (1 + (float) $product['rate'] / 100);
                                    $pa_price = $rest;
                                    $id_pa = $id['id_product_attribute'];
                                } else {
            ...
            foreach ($query_products as $query_product) {
                ...
                if ($this->not_change_final_price) {
                    ...
                    $new_tax_rule_arr = TaxRule::getTaxRulesByGroupId(
                        Configuration::get('PS_LANG_DEFAULT'),
                        (int) Tools::getValue('id_tax_rules_group'));
                    $price = (100 + $product_obj->getTaxesRate()) / (100 + $new_tax_rule_arr[0]['rate']) * $product_obj->price;
                    MassEditTools::updateObjectField(
                        'Product',
                        'price',
                        (int) $query_product['id_product'],
                ...
                if (version_compare(_PS_VERSION_, '1.7.6.9', '>=')) {
                    $return_products[$query_product['id_product']] = [
                        'price' => $this->context->currentLocale->formatPrice($price, $this->context->currency->iso_code),
                        'price_final' => $this->context->currentLocale->formatPrice($final_price, $this->context->currency->iso_code),
                        ];
                } else {

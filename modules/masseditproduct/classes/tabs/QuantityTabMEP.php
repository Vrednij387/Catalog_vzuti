...
//                }
    ...
    {
        $update_location = $this->checkAccessField('location');
        $update_quantity = $this->checkAccessField('quantity');
        $return_products = [];
...
//                    }
                    ...
                    if (!Product::usesAdvancedStockManagement((int) $id_product)
                        && $this->change_type == self::CHANGE_TYPE_QUANTITY && ($update_quantity || $update_location)) {
                        $return_products[(int) $id_product] = MassEditTools::setQuantity(
                            (int) $id_product,
                            0,
                            $update_quantity ? $this->quantity : false,
                            $this->action_quantity,
                            $id_shop,
                            $update_location ? $this->location : false
                        );
                    }

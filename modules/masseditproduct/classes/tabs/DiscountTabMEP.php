...
class DiscountTabMEP extends BaseTabMEP
{
    ...
    public $reduction_ai;
    public $reduction_type_ai;
    public $reduction_tax_ai;
    ...
    public function __construct()
    {
        ...
        $this->reduction_ai = (float) Tools::getValue('sp_reduction_ai');
        $this->reduction_type_ai = Tools::getValue('sp_reduction_type_ai');
        $this->reduction_tax_ai = Tools::getValue('sp_reduction_tax_ai');
    ...
    public function addSpecificPrice($products)
    {
        ...
        foreach ($products as $id_product) {
            ...
            if ($this->reduction_ai > 0 && $this->reduction_ai != 0) {
                if ($this->reduction_tax_ai == 0) {
                    $tax = false;
                } else {
                    $tax = true;
                }
                $price_product = Product::getPriceStatic($id_product, $tax, null, 6, null, false, false);
                if ($price_product >= $this->reduction_ai) {
                    if ($this->reduction_type_ai == 'percentage') {
                        $this->reduction = round(($price_product - $this->reduction_ai) / $price_product * 100, 2) / 100;
                        $this->reduction_type = $this->reduction_type_ai;
                        $this->reduction_tax = $this->reduction_tax_ai;
                    } else {
                        $this->reduction = $price_product - $this->reduction_ai;
                        $this->reduction_type = $this->reduction_type_ai;
                        $this->reduction_tax = $this->reduction_tax_ai;
                    }
                } else {
                    LoggerMEP::getInstance()->error(
                        sprintf($this->l('Product №%s: discount more than price.'), $id_product)
                    );
                    continue;
                }
            }
    ...
    public function updateSpecificPrice($products)
    {
        ...
        $ids_sp = Db::getInstance()->executeS('SELECT `id_specific_price` FROM `' . _DB_PREFIX_ . 'specific_price` ' . $where);

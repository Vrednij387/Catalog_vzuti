...
class ReferenceTabMEP extends BaseTabMEP
{
    ...
    {
        if ($this->assignment == 'generate_reference') {
            foreach ($this->getCombinationsFromRequest() as $key => $attr) {
                $index = 1;
                foreach ($attr as $id_attr) {
                    $sql = 'UPDATE `' . _DB_PREFIX_ . 'product_attribute` '
                        . 'SET `reference` = "' . pSQL(
                            $key . '-' . $index
                        ) . '" WHERE `id_product_attribute` = ' . $id_attr . ' AND  id_product = ' . (int) $key;
                    if (!Db::getInstance()->execute($sql)) {
                        LoggerMEP::getInstance()->error($this->l('Error writing to database'));
    ...
    public function checkBeforeChange()
    {
        ...
        if ($this->checkAccessField('selected_set_unique_reference')) {
            $this->assignment = 'generate_reference';
        }

...
class FeaturesTabMEP extends BaseTabMEP
{
    ...
    public function applyChangeForProducts($products)
    {
        ...
        $extended_features = Tools::getValue('extendedfeatures', []);
        ...
        foreach ($products as $id_product) {
            if ($multi_features) {
                if (Validate::isLoadedObject($product = new Product($id_product))) {
                    ...
                    $features = isset($form['features']) ? $form['features'] : [];
                ...
                if (Module::isEnabled('seosaextendedfeatures')) {
                    foreach ($features as $feature) {
                        ...
                        if ($delete_old['feature'][$feature['id_feature']]) {
                            MassEditTools::deleteFeatures($id_product, [$feature['id_feature']]);
                        }

                        $feature_name = str_replace(['[', ']', '(', ')', ' '], '_', $feature['name']);
    ...
    public function deleteFeaturesvalueId($id_product, $id_featurer, $id_value)
    {
        ...
        $features = Db::getInstance()->executeS(
            ...
            AND  id_feature = ' . $id_featurer . ' AND id_feature_value = ' . $id_value . !$all_shops ? ' AND fs.`id_shop` = ' . (int) Context::getContext()->shop->id : ''
        );

...
class FixtureMEP
{
    ...
    public function importEntity($class_name)
    {
        ...
        if (file_exists($fixture)) {
            $data = json_decode(Tools::file_get_contents($fixture), true);
        }
        ...
        foreach ($languages as $l) {
            $lang_fixtures[$l['iso_code']] = [];

            $fixture_lang = $fixture_lang_path . $l['iso_code'] . '/data/' . $definition['table'] . '.json';
            if (file_exists($fixture_lang)) {
                $lang_fixtures[$l['iso_code']] = json_decode(Tools::file_get_contents($fixture_lang), true);
            }
        }
        ...
        if (!Db::getInstance()->insert($definition['table'], $data, false, true, Db::INSERT_IGNORE)) {
            throw new PrestaShopException('An SQL error occurred for entity <i>%1$s</i>: <i>%2$s</i>', $definition['table'], Db::getInstance()->getMsgError());
        }
        unset($data);
        ...
        if (count($merge_data_lang)) {
            if (!Db::getInstance()->insert($definition['table'] . '_lang', $merge_data_lang, false, true, Db::INSERT_IGNORE)) {
                throw new PrestaShopException('An SQL error occurred for entity <i>%1$s</i>: <i>%2$s</i>', $definition['table'] . '_lang', Db::getInstance()->getMsgError());
            }
        }
        ...
        foreach ($data as &$row) {
            foreach ($row as $field_name => &$col) {
                ...
                if (array_key_exists($field_name, $definition['fields'])) {
                    ...
                    if (in_array($def_field['type'], [ObjectModel::TYPE_HTML, ObjectModel::TYPE_NOTHING])) {
                        $col = str_replace('\'', '\\\'', $col);
                    } else {
                        $col = pSQL($col);
                    }
        ...
        foreach ($data as &$row) {
            foreach ($row as $field_name => &$col) {
                if (array_key_exists($field_name, $definition['fields'])) {
                    ...
                    if (in_array(
                        $def_field['type'],
                        [ObjectModel::TYPE_HTML, ObjectModel::TYPE_NOTHING]
                    )) {
                        $col = str_replace('\'', '\\\'', $col);
                    } else {
                        $col = pSQL($col);

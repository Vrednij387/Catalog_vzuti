...
$.fn.selectProducts = function (p)
{
    ...
    function init()
    {
        if (defaults.search)
        {
            _this.find('.search_product').on('keyup', function () {
                searchProducts();
            });

            _this.find('[name=search_by]').on('change', function () {
                searchProducts();
            });
        }

        _this.find('.add_select_product').on('click', function () {
            var options = _this.find('.no_selected_product option:selected').remove();
            _this.find('.selected_product').append(options);
        });
        _this.find('.remove_select_product').on('click', function () {

...
//if($_SERVER['REMOTE_ADDR'] == '134.249.84.233') { var_dump($id_product_attribute, $result); }
    ...
    public static function getAttributesColorList(array $products, $have_stock = true)
    {
        ...
        $res = Db::getInstance()->executeS(
            ...
            . ($check_stock ? 'SUM(IF(stock.`quantity` > 0, 1, 0))' : '0')
            . ' qty, a.`id_attribute`, al.`name`, 
            IF(color = "", a.id_attribute, color) group_by
            FROM `' . _DB_PREFIX_ . 'product_attribute` pa
            ' . Shop::addSqlAssociation('product_attribute', 'pa')
            ...
            WHERE pa.`id_product` IN (' . implode(',', array_map('intval', $products)) . ') 
        ...
        foreach ($res as $row) {
            $row['texture'] = '';
            /* 30-08-2023 виводимо усі атрибути товару

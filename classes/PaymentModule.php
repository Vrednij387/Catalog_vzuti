...
abstract class PaymentModuleCore extends Module
{
    ...
    public function validateOrder(
        ...
        foreach ($order_detail_list as $key => $order_detail) {
            ...
            $product_var_tpl_list = $products_text_list = [];
            ...
            if ($id_order_state != Configuration::get('PS_OS_ERROR') && $id_order_state != Configuration::get('PS_OS_CANCELED') && $this->context->customer->id) 
			{
				...
				$phone_mobile = $delivery->phone_mobile;
                ...
                if (Validate::isEmail($this->context->customer->email)) {
                    $data = array(
                        ...
                        '{recycled_packaging_label}' => $order->recyclable ? $this->trans('Yes', array(), 'Shop.Theme.Global') : $this->trans('No', array(), 'Shop.Theme.Global'),
						'{bank_iban}' => Configuration::get('SHOP_BANK_IBAN'),
						'{bank_name}' => Configuration::get('SHOP_BANK_NAME'),
						'{bank_code}' => Configuration::get('SHOP_BANK_CODE'),
						'{bank_recipient}' => Configuration::get('SHOP_BANK_RECIPIENT'),
                    );
                    ...
                    Mail::Send(
                        (int) $order->id_lang,
                        (($order->module === 'ps_cashondelivery' || $order->module === 'ps_cashondelivery2') ? 'order_conf_bank_info' : 'order_conf'),
                        $this->context->getTranslator()->trans(
                            'Замовлення у Vzuti №' . $order->id,
                            array(),
                            'Emails.Subject',
				...
				// Vzuti - ФлайСМС - вайбер-сповіщення по замовленню
				if($order->module === 'ps_cashondelivery' && false) // післяоплата
				{
					$cms_7_content = CMS::getCMSContent(7, $order->id_lang, $order->id_shop);
				
									$products = array();
									foreach($products_text_list as $product)
									{
										$products[] = '📦 Товар: '.$product['name'].'
					🔖 Артикул: '.$product['reference'].'
					💵 Ціна: '.$product['price'];
					}
									
									$text = 'Вітаємо!
					📤 Ваш номер замовлення: '.$order->id.'
					'.implode("\r\n", $products).'
					
					'.$cms_7_content['content'].'
					
					‼️Призначення платежу: оплата за товар №'.$order->id.'
					
					📩Квитанцію про оплату надішліть, будь ласка, на вайбер 0502492615';
					//echo $text;
					
					$fly_response = $this->sendToAPI($phone_mobile, $text);
					
					if($fly_response)
					{
						$res_update = $order->setFly($fly_response);
					}

				}
				// end Vzuti - ФлайСМС - вайбер-сповіщення по замовленню
            }
	...
	private	function sendToAPI($phone_mobile, $text)
	{
	
		$data = array(
			'auth' => array('key' => 'CnpS4UR6tkoka45gS95gtS1Kcr5xodmb'),
			'action' => 'SENDMESSAGE',
			'data' => array(
		...
		CURLOPT_URL => 'https://sms-fly.ua/api/v2/api.php',
